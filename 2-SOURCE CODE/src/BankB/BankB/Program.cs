...
var bankCode = Environment.GetEnvironmentVariable("Bank__Code") ?? "BANKB";
        ...
        try
        {
            ...
            if (count == 0)
            {
                db.Accounts.AddRange(
                    new Account { AccountNumber = "BANKB001", AccountHolderName = "Alice Williams", Balance = 100000, CreatedAt = DateTime.UtcNow },
                    new Account { AccountNumber = "BANKB002", AccountHolderName = "Charlie Brown", Balance = 50000, CreatedAt = DateTime.UtcNow },
                    new Account { AccountNumber = "BANKB003", AccountHolderName = "Diana Prince", Balance = 75000, CreatedAt = DateTime.UtcNow }
                );
                db.SaveChanges();
...
// Health check
app.MapGet("/health", () => Results.Ok(new { status = "healthy", service = "bank-b", bankCode }));
