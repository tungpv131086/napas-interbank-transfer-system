...
var bankCode = Environment.GetEnvironmentVariable("Bank__Code") ?? "BANKC";
        ...
        try
        {
            ...
            if (count == 0)
            {
                db.Accounts.AddRange(
                    new Account { AccountNumber = "BANKC001", AccountHolderName = "Eve Anderson", Balance = 100000, CreatedAt = DateTime.UtcNow },
                    new Account { AccountNumber = "BANKC002", AccountHolderName = "Frank Miller", Balance = 50000, CreatedAt = DateTime.UtcNow },
                    new Account { AccountNumber = "BANKC003", AccountHolderName = "Grace Lee", Balance = 75000, CreatedAt = DateTime.UtcNow }
                );
                db.SaveChanges();
...
// Health check
app.MapGet("/health", () => Results.Ok(new { status = "healthy", service = "bank-c", bankCode }));
